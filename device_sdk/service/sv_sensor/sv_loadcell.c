...
int8_t sv_lc_tare(sv_loadcell_t* _this){
    ...
    _this->m_offset_val = cal_mass(_this);
...
static int32_t cal_mass(sv_loadcell_t*_this){
    int32_t raw_data = 0;
    hx711_read_average(_this->m_adc, 3, &raw_data);
    if (_this->m_calib_val == 0)
        sv_lc_calib(_this);
        
    raw_data = raw_data / _this->m_calib_val;

    if (_this->m_offset_val != 0)
        raw_data = raw_data - _this->m_offset_val;
